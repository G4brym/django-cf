    ...
    def run_query(self, query, params=None) -> CFResult:
        ...
        try:
            if read_only:
                ...
                result = CFResult.from_dict(query, params, response, len(response), 0)
            else:
                response = self.run_sync(stmt.all())
                result = CFResult.from_dict(query, params, response.results.to_py(), response.meta.rows_read, response.meta.rows_written,
